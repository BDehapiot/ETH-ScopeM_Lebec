...
def l5p_plot(fit, display=False, save=True):
    
    # Fetch
    y = fit["y"]
    popt = fit["popt"]
    fA, fD, fC, fB, fE = fit["params"]
    t05, t50, t95 = fit["tmarks"]
...
if __name__ == "__main__":
    ...
    # Fit
    fit = l5p_fit(np.mean(prd, axis=(1, 2)), plot=True)
...
#%% l5p fit -------------------------------------------------------------------
    ...
    # t05 = l5p_y2x(fA + (fD - fA) * 0.05, popt, x_bounds=(0, len(y)))
    # t50 = l5p_y2x((fD - fA) / 2, popt, x_bounds=(0, len(y)))
    # t95 = l5p_y2x(fA + (fD - fA) * 0.95, popt, x_bounds=(0, len(y)))
...
#%% Plot ----------------------------------------------------------------------
    ...
    # axis.axvline(
    #     x=t05, ymin=fApc, ymax=fDpc, color="k", linestyle="--", linewidth=0.5)
    # axis.axvline(
    #     x=t50, ymin=fApc, ymax=fDpc, color="k", linestyle="-" , linewidth=1)
    # axis.axvline(
    #     x=t95, ymin=fApc, ymax=fDpc, color="k", linestyle="--", linewidth=0.5)
    # axis.axhline(y=fA , color="k", linestyle="--", linewidth=0.5)
    # axis.axhline(y=fD , color="k", linestyle="--", linewidth=0.5)
