...
#%% Method : predict() --------------------------------------------------------
    ...
    def predict(self):
        ...
        for path in stk_paths:
            stks.append(io.imread(path))
        
        # Predict        
        prds = []
        for i in range(0, len(stks), self.batch_size):
            sub_stks = np.stack(stks[i:i + self.batch_size])
            shape0 = sub_stks.shape
            shape1 = (shape0[0] * shape0[1],) + shape0[2:]
            sub_stks = sub_stks.reshape(shape1)
            sub_prds = unet.predict(np.stack(sub_stks), verbose=1)
            sub_prds = sub_prds.reshape(shape0)
            sub_prds = [sub_prds[j] for j in range(sub_prds.shape[0])]
            prds = prds + sub_prds
...
#%% Method : fit() ------------------------------------------------------------
    ...
    def fit(self):
        ...
        def plot():
            ...
            plt.tight_layout()
            plt.savefig(
                self.out_path / (self.snames[i] + "_plot.png"), format="png")
            plt.close(fig)
        ...
        for i, prd in enumerate(prds):
            ...
            # Save
            pkl_path = self.out_path / (self.snames[i] + "_fit_data.pkl")
    ...
    def analyse(self):
        ...
        for i, path in enumerate(pkl_paths):
            ...
            with open(str(path), "rb") as f:
                self.fit_data.append(pickle.load(f))
...
if __name__ == "__main__":
    ...
    avg_data, t05ss, t50ss, t95ss, yss  = [], [], [], [], []
    ...
    # Plot - line plots - Avg. prob
    ax0 = fig.add_subplot(gs[0, :2])
    for i, cond in enumerate(unique_conds):
        ls = "-" if cond[1] == "para" else  "--"
        color = cmap(i) if cond[1] == "para" else cmap(i - 1)
        ax0.plot(
            avg_data[i]["y_avg"], ls, lw=2, color=color, alpha=1.0, 
